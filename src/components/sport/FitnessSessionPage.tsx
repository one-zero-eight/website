...
import {
  FitnessTestSessionDetails,
  FitnessTestStudentSuggestion,
  fitnessTestAPI,
} from "./services/fitnessTestAPI";
import StudentSearch from "./StudentSearch";
import StudentResultsForm from "./StudentResultsForm";
import ResultsTable from "./ResultsTable";

const FitnessSessionPage: React.FC<{ session: FitnessTestSessionDetails }> = ({
  session,
}) => {
  ...
  const [selectedStudent, setSelectedStudent] =
    useState<FitnessTestStudentSuggestion | null>(null);
  // studentResults: { [studentId: string]: { [exerciseId: number]: string | number } }
  const [studentResults, setStudentResults] = useState<
    Record<string, Record<number, string | number>>
  >({});
  ...
  const debugSubmitResults = async () => {
    ...
    const results = Object.entries(studentResults).map(
      ([exercise_id, value]) => ({
  ...
  React.useEffect(() => {
    ...
    if (exerciseResults.length > 0) {
      setStudentResults((prev) => ({
        ...prev,
        [studentId]: exerciseResults.reduce(
          (acc, ex) => {
  ...
  const [editStatus] = useState<"idle" | "loading" | "success" | "error">(
    "idle",
  );
  ...
  return (
    <div className="mx-auto max-w-5xl py-8">
      <h1 className="mb-6 text-2xl font-bold">
        Fitness Test Session: {session.session.semester.name}
      </h1>
      <div className="mb-4 text-sm text-inactive">
        {new Date(session.session.date).toLocaleString()} | Teacher:{" "}
      ...
      {selectedStudent && (
        <StudentResultsForm
          ...
          studentResults={
            studentResults[selectedStudent.value.split("_")[0]] || {}
          }
      ...
      <div className="mt-6 flex justify-end">
        <button
          className="innohassle-button rounded px-6 py-2 text-base"
          ...
          onClick={async () => {
            ...
            try {
              ...
              Object.entries(studentResults).forEach(
                ([student_id, exercises]) => {
        ...
        <button
          className="innohassle-button ml-4 rounded bg-red-200 px-4 py-1 text-xs"
