...
export function RoomMapPreview({ roomId }: { roomId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="skeleton relative min-h-64 w-full overflow-hidden border shadow-xs"></div>
    );
  }

  if (!sceneWithRoom || !roomArea) {
    return (
      <div className="border-border bg-muted/30 rounded-box flex min-h-64 w-full items-center justify-center border">
  ...
  return (
    <div className="border-border bg-background rounded-box relative min-h-64 w-full overflow-hidden border shadow-xs">
      <MapView
        scene={sceneWithRoom}
